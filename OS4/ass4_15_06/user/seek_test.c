...
int test_write(int fd, int offset, int whence, char *toWrite, int length)
{
    printf("test write:%d %d %d %d %d\n", fd, offset, whence, toWrite, length);
...
int test_read(int fd, int offset, int whence, char *expected, int length)
{
    if (seek(fd, offset, whence) < 0)
        return 0;
    char buf[length];
...
void handleFailureSeek(char *errorMsg, int fd)
{
    printf("%s\n", errorMsg);
    close(fd);
}

int openTestFile(void)
{
...
void print_file_contents(int fd, int length, char *msg)
{
    char buf[length];
    seek(fd, 0, SEEK_SET);
    read(fd, buf, length);
...
int test_seek(int print)
{
    ...
    {
        if (print)
            printf("SUCCESS! Successfully written \"Hello, World!\\n\" to the file\n");
    }
    else
    {
        if (res1 == 0)
    ...
    {
        if (print)
            printf("SUCCESS! Successfully written \"This is new data!\\n\" to the file\n");
    }
    else
    {
        if (res2 == 0)
    ...
    {
        if (print)
            printf("SUCCESS! Successfully read exactly the last 5 characters from the file\n");
    }
    else
    {
    ...
    {
        if (print)
            printf("SUCCESS! Successfully written \"Extra fresh data!\\n\" to the file\n");
    }
    else
    {
        if (res3 == 0)
